...
def recognize_captcha(page) -> str | None:
    """截图验证码 → 德勤OCR识别"""
    ...
    try:
        ...
        data = resp.json()
        texts = []
        for table in data.get("result", {}).get("tables", []):
...
def handle_captcha_flow(page) -> bool:
    """
    EasyOCR 识别验证码（区分大小写）
    """
...
def process_single_code(page, code: str, index: int) -> dict | None:
    """处理单条HS编码的完整流程"""
    log.info(f"{'='*50}")
    ...
    for query_attempt in range(1, QUERY_MAX_RETRIES + 1):
        ...
        if not handle_captcha_flow(page):
    ...
    with sync_playwright() as p:
        ...
        try:
            for i, code in enumerate(remaining, start=1):
                ...
                result = process_single_code(page, code, global_index)
