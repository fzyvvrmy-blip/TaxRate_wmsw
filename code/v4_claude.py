"""
商务部外贸实务查询平台 — HS编码关税爬虫 V4 (Claude Opus OCR)
============================================================
用法:
    python v4_claude.py "CSV文件路径"

使用 Claude Opus 4.7 视觉识别验证码。
...
CLAUDE_API_URL = "https://api.kyapihub.com/v1/messages"
CLAUDE_API_KEY = "sk-yuuDqcTs4262PbFlwhIxgeSPWTZs28UKd1QToE5RqnO3Wicg"
CLAUDE_MODEL = "claude-opus-4-8"


def ocr_captcha(img_bytes: bytes, _=None) -> str | None:
    """Claude 识别验证码（参考 openai-captcha-detection 方案：缩小+中文prompt）"""
    try:
        img = Image.open(io.BytesIO(img_bytes)).convert("RGB")
        img.thumbnail((300, 100))  # 等比缩放到300x100内
        buf = io.BytesIO()
        img.save(buf, format="PNG")
        img_b64 = base64.b64encode(buf.getvalue()).decode()

        resp = requests.post(CLAUDE_API_URL,
            ...
            json={"model": CLAUDE_MODEL, "max_tokens": 300,
                  "messages": [{"role": "user", "content": [
                      ...
                      {"type": "text", "text": "请对这张图片进行OCR识别，输出验证码的4个字符，不要输出其他内容。"}
                  ]}]}, timeout=30)
        data = resp.json()
        result = data["content"][0]["text"].strip()
        result = re.sub(r"[\s\n\r\t\-]", "", result)
        log.info(f"Claude OCR: '{result}' ({len(result)}位)")
        return result if len(result) == 4 else None
    except Exception as e:
        log.error(f"Claude OCR异常: {e}")
        return None
